...
_TOTAL_ROWS = 198
assert 0 <= START_ROW < _TOTAL_ROWS, f"START_ROW doit être entre 0 et {_TOTAL_ROWS-1}"
assert START_ROW < END_ROW <= _TOTAL_ROWS, f"END_ROW doit être entre {START_ROW+1} et {_TOTAL_ROWS}"
 
TEST_MODE      = True
TEST_MAX_JOBS  = END_ROW - START_ROW   
TEST_MAX_CVS   = 5
...
GRAPHS_DIR   = "C:\\Users\\USER\\Desktop\\sou\\final1v"
RESULTS_DIR  = "C:\\Users\\USER\\Desktop\\sou\\final2v2"
BIAS_DIR     = "C:\\Users\\USER\\Desktop\\sou\\final3v2"
REPORTS_DIR  = "C:\\Users\\USER\\Desktop\\sou\\final4v2"
...
print(f"   Plage lignes : {START_ROW} → {END_ROW-1}  ({END_ROW - START_ROW} offres)")
if USE_NVIDIA_API:
...
def _clean_json_response(raw: str) -> str:
    """Nettoie et répare le JSON du LLM — gère le texte narratif Ollama/Gemma3."""
    if not raw:
        return "{}"
...
def _extract_row_idx_from_job_id(job_id: str, df: pd.DataFrame) -> int | None:
    """Extrait l'index de ligne depuis job_id de façon robuste.
    Gère : 'job_0', 'job_42', 'job_mon_entreprise_5', etc.
    """
    ...
    try:
        matches = df.index[df.index == int(job_id.split("_")[-1])].tolist()
        if matches:
            return matches[0]
    except (ValueError, TypeError):
        pass
 
    print(f"  ⚠ Impossible d'extraire row_idx depuis job_id='{job_id}'")
    return None
...
def run_phase1_graph_generation(
    ...
    print(f"█  Mode : {'🧪 TEST' if test_mode else '🚀 COMPLET'}")
    print(f"█  Lignes : {start_row} → {end_row - 1}  ({end_row - start_row} offres)")
    print("█" * 70)
    ...
    if test_mode:
        ...
        print(f"  🧪 TEST : lignes {start_row}→{end_row - 1} ({len(df)} jobs)")
    else:
        df = df.iloc[start_row:end_row]
        print(f"  📋 Plage : lignes {start_row}→{end_row - 1} ({len(df)} offres)")
...
class CvLLM:
    ...
    def _repair_json(self, text: str) -> str:
        # ✅ FIX 3 appliqué ici aussi : utilise la même fonction centrale
...
"""
        ...
        try:
            if not raw:
                raise TypeError("LLM returned None or empty string")
            data = json.loads(raw)
...
def score_edges_phase3(
    ...
    for i in range(0, total_edges, batch_size):
        ...
        for edge in batch:
            ...
            # Tell the LLM to use simple "source|target" id keys
            edge_copy["_expected_key"] = f"{src}|{tgt}"
            enriched_batch.append(edge_copy)
 
           
            phase2_key = None
            for k, v in edge_instructions.items():
                if v.get("source") == src and v.get("target") == tgt:
                    phase2_key = k
                    break
            if phase2_key is None:
                phase2_key = f"{src}|{tgt}"
 
            
            key_remap[f"{src}|{tgt}"]  = phase2_key
            key_remap[f"{src}->{tgt}"] = phase2_key
            
            src_label = scored_nodes.get(src, {}).get("label", src)
            tgt_label = scored_nodes.get(tgt, {}).get("label", tgt)
            key_remap[f"{src}|{src_label}|{tgt}|{tgt_label}"] = phase2_key
            key_remap[f"{src_label}|{tgt_label}"]             = phase2_key
...
Use the key format: "<source_id>|<target_id>" exactly as shown in _expected_key fields.
"""
        ...
        try:
            ...
            remapped_count = 0
            for llm_key, ed in scored.items():
                # Remap LLM key to canonical Phase2 key
                canonical_key = key_remap.get(llm_key, key_remap.get(llm_key.replace("->", "|"), llm_key))
                ...
                all_scored_edges[canonical_key] = ed
                remapped_count += 1
            print(f"✓ {remapped_count}/{len(batch)}")
        except Exception as e:
            print(f"✗ Erreur: {str(e)[:40]}")
            for edge in batch:
                src = edge.get("source", "")
                tgt = edge.get("target", "")
                # Find original phase2 key
                phase2_key = key_remap.get(f"{src}|{tgt}", f"{src}|{tgt}")
                all_scored_edges[phase2_key] = {
                    "synergy_score": 0.0,
                    "w_cv": 0.0,
                    "evidence_quality": "absent",
                    "score_breakdown": {
...
def audit_edge_bias(
    ...
    ideal_map = {}
    for e in ideal_edges:
        src = e.get("source", "")
        tgt = e.get("target", "")
        ideal_map[f"{src}|{tgt}"] = e      
        ideal_map[f"{src}->{tgt}"] = e     
 
    for edge_key, edge_score in scored_edges.items():
        
        normalized_key = edge_key.replace("->", "|")
        ideal_edge = ideal_map.get(normalized_key) or ideal_map.get(edge_key)
        if not ideal_edge:
            continue
...
def run_phase3_5(
    ...
    for pair_key, phase3_output in PHASE3_STORE.items():
        ...
        row_idx = _extract_row_idx_from_job_id(job_id, df)
        if row_idx is None:
            print(f"  ⚠ {pair_key} — impossible d'extraire row_idx depuis '{job_id}', skip")
            continue
        if row_idx not in df.index:
            print(f"  ⏩ {pair_key} — index {row_idx} absent du DataFrame (plage {df.index[0]}→{df.index[-1]})")
            continue
...
def _edge_wcv_norm(edge: dict, cv_scores: Dict[str, dict], scored_edges: Optional[Dict[str, dict]] = None) -> float:
   
    if scored_edges is not None:
        src = edge.get("source", "")
        tgt = edge.get("target", "")
        
        for key in (f"{src}|{tgt}", f"{src}->{tgt}"):
            entry = scored_edges.get(key)
            if entry is not None:
                return float(entry.get("w_cv", entry.get("synergy_score", 0.0))) / _MAX_CV_SCORE
    
    src = edge.get("source", "")
    tgt = edge.get("target", "")
    w_src = float(cv_scores.get(src, {}).get("w_cv", 0.0)) / _MAX_CV_SCORE
    w_tgt = float(cv_scores.get(tgt, {}).get("w_cv", 0.0)) / _MAX_CV_SCORE
    return (w_src + w_tgt) / 2.0
...
def compute_edge_metrics(edges: List[dict], cv_scores: Dict[str, dict], scored_edges: Optional[Dict[str, dict]] = None) -> dict:
    ...
    for edge in edges:
        ...
        w0      = _w0(edge)
        wcvn    = _edge_wcv_norm(edge, cv_scores, scored_edges)
        wcv_raw = wcvn * _MAX_CV_SCORE
 
        
        edge_key_pipe = f"{source}|{target}"
        scored_entry  = scored_edges.get(edge_key_pipe, {})
 
        edge_scores.append({
            "edge_key":      edge_key_pipe,
            "source":        source,
            "target":        target,
            "w_0":           round(w0, 3),
            ...
            "p3_confidence": scored_entry.get("p3_confidence", edge.get("p3_confidence", "NONE")),
            "p35_is_biased": scored_entry.get("p35_is_biased", edge.get("p35_is_biased", False)),
        })
...
def extract_bias_summary(
    ...
    for edge in edges:
        ...
        ekey_pipe   = f"{source}|{target}"
        ekey_arrow  = f"{source}->{target}"
        sc = scored_edges.get(ekey_pipe, scored_edges.get(ekey_arrow, {}))
...
def compute_final_candidate_score(
    ...
    edge_scores_list, edge_weights = [], []
    scored_edges = scored_edges or {}
    for edge in edges:
        w0 = _w0(edge)
        wcv = _edge_wcv_norm(edge, cv_scores, scored_edges) * _MAX_CV_SCORE
        edge_scores_list.append(wcv * w0)
        edge_weights.append(w0)
 
    node_score = sum(node_scores) / sum(node_weights) if node_weights else 0.0
    edge_score = sum(edge_scores_list) / sum(edge_weights) if edge_weights else 0.0
...
def compute_advanced_metrics_v6(
    ...
    if edges:
        ...
        for edge in edges:
            ...
            edge_key = f"{src}|{tgt}"
            scored_entry = scored_edges.get(edge_key, {})
            if scored_entry:
                synergy = float(scored_entry.get("w_cv", scored_entry.get("synergy_score", 0.0)))
                match = min(synergy / _MAX_CV_SCORE, 1.0)
            else:
...
def phase4_generate_report(
    ...
    scored_edges: Dict[str, dict] = phase3_data.get("scored_edges", {})
    ...
    edge_metrics = compute_edge_metrics(edges, cv_scores, scored_edges)
    bias_summary = extract_bias_summary(ideal_nodes, edges, cv_scores, scored_edges)
    profile      = compute_profile_summary(ideal_nodes, cv_scores, gap_data)
    final_scores = compute_final_candidate_score(ideal_nodes, edges, cv_scores, scored_edges)
    ...
    v6_metrics = compute_advanced_metrics_v6(ideal_nodes, edges, cv_scores, scored_edges)
    ...
    return {
        ...
        "advanced_metrics_v6": {
            ...
            "jaccard_nodes":            {"value": v6_metrics["jaccard_nodes"], "description": "Jaccard(skills couverts CV, skills requis JD)"},
            ...
            "redundancy_penalty":  {"value": v6_metrics["redundancy_penalty"],  "description": "Pénalité surqualification inutile (1=aucune redondance)"},
            "redundancy_info":     {"value": v6_metrics["redundancy_info"],      "label": ("Profil sur-qualifié sur quelques skills" if v6_metrics["redundancy_info"] > 0.15 else "Profil bien calibré"), "note": "Informatif uniquement"},
        ...
        "combo_scores_v6": {
            "combo_v6_final":  {"value": combo_v6_final,   "description": "Score synthèse V6"},
            "combo_matching":  {"value": combo_matching,   "description": "Matching pur (node + edge + jaccard)"},
            "combo_fit":       {"value": combo_fit,        "description": "Qualité de fit (gap + semantic + jaccard)"},
            "combo_ranking":   {"value": combo_ranking,    "description": "Ranking-aware (ndcg + map + mrr)"},
            "combo_prf1":      {"value": combo_prf1,       "description": "F1 pondéré + profondeur"},
            "combo_safe":      {"value": combo_safe,       "description": "Candidat sans risque (pénalités + gap)"},
            "combo_specialist":{"value": combo_specialist, "description": "Top alignement + profondeur"},
        },
        ...
        "critical_gaps": [
            {"skill": g["label"], "type": g["type"], "gap": g["gap"],
             "severity": g["severity"], "importance": g["w_0"]}
            for g in gap_data["critical_gaps"][:10]
        ],
 
        "profile_summary": {
            "strengths":           [{"label": s["label"], "type": s["type"], "w_cv": s["w_cv"]} for s in profile["strengths"]],
            "critical_weaknesses": [{"label": w["label"], "type": w["type"], "gap": w["gap"]} for w in profile["critical_weaknesses"]],
        },
        ...
        "decision_rationale": {
            ...
            "v6_summary": (
                f"weighted_f1={v6_metrics['weighted_f1']}, gap_score={v6_metrics['gap_score']}, "
                f"ndcg_5={v6_metrics['ndcg_5']}, missing_critical_penalty={v6_metrics['missing_critical_penalty']}, "
