# -*- coding: utf-8 -*-
"""llama70bvfinal.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/14PpqAazKzMQrzrS02byWkBfu-wlx4FTx
"""

!pip install -q spacy pymupdf pandas networkx huggingface_hub
!pip install -q gliner gliner2 transformers torch groq
...
DRIVE_PIPELINE_FOLDER = "cv_hugpro"
drive.mount('/content/drive')
CSV_FILENAME          = "job (1).csv"

DRIVE_BASE     = f"/content/drive/MyDrive/{DRIVE_PIPELINE_FOLDER}"
COLAB_WORK     = "/content/cv_nvidia1"
CSV_INPUT      = f"{DRIVE_BASE}/{CSV_FILENAME}"
GRAPHS_DIR     = f"{DRIVE_BASE}/71-80/graphs"
RESULTS_DIR    = f"{DRIVE_BASE}/71-80/results"
BIAS_DIR       = f"{DRIVE_BASE}/71-80/bias"
REPORTS_DIR    = f"{DRIVE_BASE}/71-80/reports"
...
JOB_ROW_INDEX: Dict[str, int] = {}

print("✅ Config chargée (HUGGING FACE API).")
...
print(f"   job_ids attendus : job_{START_ROW} … job_{END_ROW-1}")
...
HUGGING_API_KEYS: List[str] = [

    ""
]

HF_TOKEN   = next((k for k in HUGGING_API_KEYS if k.strip()), "")
HF_API_KEY = HF_TOKEN
...
class HuggingFaceClient:
    """
    Client HuggingFace avec rotation automatique de clés API.
    — Aucune troncature des messages.
    — Rotation immédiate sur 401/403/429/quota/expired.
    — Retry sur la même clé pour timeout/503.
    — RuntimeError uniquement quand toutes les clés sont épuisées.
    """
    MAX_RETRIES_SAME_KEY = 3
    ...
    def __init__(
        ...
        if api_keys:
            ...
            self._keys = [k.strip() for k in HUGGING_API_KEYS if k and k.strip()]

        if not self._keys:
            raise ValueError("❌ Aucune clé HuggingFace valide fournie.")

        try:
            from huggingface_hub import InferenceClient
            self._InferenceClient = InferenceClient
        except ImportError:
            raise ImportError("❌ huggingface_hub non installé → pip install huggingface_hub")
        ...
        print(f"  ✅ HuggingFaceClient initialisé — {len(self._keys)} clé(s) | modèle: {self.model}")
        self._log_active_key()
    ...
    def _get_client(self, idx: int):
        if idx not in self._clients:
            self._clients[idx] = self._InferenceClient(
                model=self.model,
                token=self._keys[idx],
                timeout=90,
            )
        return self._clients[idx]

    def _log_active_key(self):
        key    = self.api_key
        masked = f"{key[:8]}...{key[-4:]}" if len(key) > 12 else "****"
        print(f"  🔑 Clé active : #{self._current_idx + 1}/{len(self._keys)} ({masked})")
    ...
    def _rotate_key(self, reason: str) -> bool:
        ...
        if not available:
            return False
        self._current_idx = available[0]
        print(f"\n  🔄 Rotation de clé ({reason})")
        self._log_active_key()
        return True
    ...
    def chat_completion(
        ...
        while total_rotations <= n_keys:
            client            = self._get_client(self._current_idx)
            same_key_attempts = 0

            while same_key_attempts < self.MAX_RETRIES_SAME_KEY:
                same_key_attempts += 1
                try:
                    ...
                    if any(t in err for t in _ROTATE_TRIGGERS):
                        print(f"  ⚠  Clé #{self._current_idx + 1} rejetée : {str(e)[:80]}")
                        rotated = self._rotate_key(str(e)[:60])
                        if rotated:
                            total_rotations  += 1
                            client            = self._get_client(self._current_idx)
                            same_key_attempts = 0
                            ...
                            raise RuntimeError(
                                f"❌ Toutes les clés HuggingFace ({n_keys}) sont épuisées.\n"
                                f"   Dernière erreur : {e}"
                            )
                        break

                    elif any(t in err for t in _RETRY_SAME_KEY):
                        wait = 10 * same_key_attempts
                        print(
                            f"  ⏳ Clé #{self._current_idx + 1} timeout "
                            f"({same_key_attempts}/{self.MAX_RETRIES_SAME_KEY}) "
                            f"— attente {wait}s..."
                        )
                        time.sleep(wait)

                    else:
                        print(f"  ❌ Erreur inconnue clé #{self._current_idx + 1}: {str(e)[:100]}")
                        if same_key_attempts >= self.MAX_RETRIES_SAME_KEY:
                            rotated = self._rotate_key(f"erreur inconnue: {str(e)[:40]}")
                            if rotated:
                                total_rotations  += 1
                                client            = self._get_client(self._current_idx)
                                same_key_attempts = 0
                                ...
                                raise RuntimeError(
                                    f"❌ Toutes les clés HuggingFace épuisées. Dernière erreur : {e}"
                                )
                            break
                        else:
                            time.sleep(5 * same_key_attempts)

        raise RuntimeError(
            f"❌ Impossible d'obtenir une réponse après rotation de {n_keys} clé(s)."
        )


def make_llm_client(model: str = None) -> HuggingFaceClient:
    return HuggingFaceClient(api_keys=HUGGING_API_KEYS, model=HF_MODEL)
...
def llm_pass_a(jd_text: str, client: HuggingFaceClient,
               candidates: list[dict], job_id: str) -> list[dict]:
    all_nodes = []
    batch_size = 15
    target_candidates = candidates
    for i in range(0, len(target_candidates), batch_size):
        batch = target_candidates[i:i+batch_size]
        print(f"    [Pass A] Lot {i//batch_size + 1}/{(len(target_candidates)-1)//batch_size + 1}")
        hints_str = "\n".join(
...
def llm_pass_b(nodes: list[dict], jd_text: str,
               client: HuggingFaceClient, job_id: str) -> list[dict]:
    if not nodes:
        return []
        ...
        for n2 in top_nodes[i+1:]:
            if n1.get("type") != n2.get("type"):
                edges.append({"source": n1["id"],"target": n2["id"],
                               ...
                               "rationale": "Auto-généré (fallback)"})
...
def phase1_generate_graph(jd_text: str, extractor: FusedExtractor,
                           llm_client: HuggingFaceClient, vocab: VocabManager,
                           registry: SkillRegistry, job_id: str = "unknown",
                           ) -> tuple[nx.DiGraph, dict | None]:
    print(f"\n{'─'*50}")
    print(f"  [Phase 1] {job_id}")
    ...
    if skipped:
        print(f"  ⚠️ {skipped} arête(s) ignorée(s) (nœuds introuvables)")
...
def _make_safe_job_id(row: pd.Series, id_column, abs_idx: int) -> str:
...
def run_phase1_graph_generation(
    ...
    print(f"█  job_ids : job_{start_row} … job_{end_row - 1}")
    print("█" * 70)

    global JOB_ROW_INDEX
    ...
    print(f"  Plage active : lignes {start_row}→{end_row - 1} ({len(df_slice)} jobs)")
    ...
    for idx, row in df_slice.iterrows():
        ...
        JOB_ROW_INDEX[job_id] = df.index.get_loc(idx)
...
class CvLLM:
    def __init__(self, model: str = None):
        self.client = HuggingFaceClient(api_keys=HUGGING_API_KEYS, model=HF_MODEL)
        self.model  = HF_MODEL
...
def build_graph_from_data(job_data: dict) -> nx.DiGraph:
    ...
    for n in job_data.get("nodes", []):
        G.add_node(n["id"], label=n.get("label", n["id"]),
                   type=n.get("type","Hard Skill"), w_0=n.get("w_0", 0.5),
                   seniority=n.get("seniority_required","any"))
    for e in job_data.get("edges", []):
        G.add_edge(e["source"], e["target"],
                   weight=e.get("weight", 0.5), relation=e.get("relation","related"))
    return G


def _normalize_edge_key(source: str, target: str) -> str:
    return f"{source}|{target}"


def _lookup_edge(scored_edges: Dict[str, dict], source: str, target: str) -> dict:
    for key in (f"{source}|{target}", f"{source}->{target}"):
        if key in scored_edges:
            return scored_edges[key]
    return {}


def extract_edges_instructions(edges: list[dict], node_info: dict,
                                cv_text: str, llm: CvLLM) -> dict:
    if not edges:
        return {}
        ...
        try:
            ...
            for item in edge_instr:
                if isinstance(item, dict):
                    src = item.get("source", "")
                    tgt = item.get("target", "")
                    key = _normalize_edge_key(src, tgt)
...
def run_phase2_audit_strategy(job_data: dict, cv_text: str, llm: CvLLM,
                               job_id: str = "unknown", cv_id: str = "cv_1") -> dict:
...
"""
        ...
        try:
            ...
            batch_expected_ids = [n["id"] for n in batch]
            validated_map  = validate_and_fix_skill_ids(batch_instr, batch_expected_ids, auto_fix=True)
            validated_instr = list(validated_map.values())
            all_instructions.extend(validated_instr)
...
def run_phase2(
    ...
    if not GRAPHS_STORE:
        print("  ❌ GRAPHS_STORE vide — lancez run_phase1 d'abord")
        return {}
    ...
    for idx, row in df_slice.iterrows():
        ...
        if not job_data:
            print(f"  ⏩ {job_id} — graphe manquant dans GRAPHS_STORE")
            continue

        cv_cols_to_process = cv_columns[:max_cvs]
    ...
    print(f"\n  ✅ Phase 2 : {len(results)} paires → PHASE2_STORE")
    return results
...
def run_phase3_scoring(phase2_output: dict, llm: CvLLM) -> dict:
...
"""
    ...
    if edge_instructions:
        all_scored_edges = score_edges_phase3(
            edge_instructions, all_scored_nodes, cv_sections, llm)
...
def score_edges_phase3(edge_instructions: dict, scored_nodes: dict,
                        cv_sections: dict, llm: CvLLM) -> dict:
    ...
    for i in range(0, total_edges, batch_size):
        ...
        for edge in batch:
            ...
            tgt = edge.get("target")
            ec  = edge.copy()
            ec["parent_source_score"] = scored_nodes.get(src, {}).get("w_cv", 0.0)
            ec["parent_target_score"] = scored_nodes.get(tgt, {}).get("w_cv", 0.0)
...
"""
        ...
        try:
            ...
            for ekey, ed in scored.items():
                ...
                if len(parts) == 2:
                    canonical_key = _normalize_edge_key(parts[0].strip(), parts[1].strip())
                else:
                    canonical_key = ekey
                synergy = max(0.0, min(1.0, float(ed.get("synergy_score", 0.0))))
                ed["synergy_score"] = synergy
                ed["w_cv"] = synergy
            ...
            for edge in batch:
                ...
                all_scored_edges[key] = {
                    ...
                    "is_ghost_synergy": False,
                    "rationale": "Scoring error - fallback",
                    "p3_confidence": "NONE","p3_source": "fallback"}
        ...
        if llm_exp_bonus == 0.0:
            ...
            for exp_skill, years in exp_years.items():
                if exp_skill in skill_label or skill_label in exp_skill:
                    exp_bonus_from_regex = min(int(years) * 0.05, 0.30)
                    break
            if exp_bonus_from_regex > 0:
                ...
                try:
                    llm_years = float(llm_years) if llm_years else 0.0
...
def run_phase3(
    ...
    if not PHASE2_STORE:
        print("  ❌ PHASE2_STORE vide — lancez run_phase2 d'abord")
        return {}
    ...
    for pair_key, phase2_output in PHASE2_STORE.items():
        ...
        result["scored_nodes"] = apply_experience_and_cert_bonus(
            result["scored_nodes"],
            phase2_output.get("skill_instructions", {}),
            phase2_output.get("exp_years", {}),
        )

        results[pair_key]      = result
        PHASE3_STORE[pair_key] = result

        out_path = os.path.join(output_dir, f"{pair_key}_phase3.json")
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(result, f, indent=2, ensure_ascii=False)

    print(f"\n  ✅ Phase 3 : {len(results)} paires → PHASE3_STORE")
    return results
...
def _audit_node_bias(node_id: str, node_data: dict, w0: float,
                     w_cv: float, context: str, llm) -> dict | None:
...
def audit_edge_bias(scored_edges: Dict[str, dict], ideal_edges: list,
                    context: str, llm) -> Dict[str, dict]:
    audited = {}
    for ideal_edge in ideal_edges:
        src = ideal_edge.get("source","")
        tgt = ideal_edge.get("target","")
        edge_score = _lookup_edge(scored_edges, src, tgt)
        if not edge_score:
            continue
        if edge_score.get("p3_source") == "fallback":
            continue
        ...
        if w_cv < 0.1 or delta < BIAS_AUDIT_DELTA_THRESHOLD:
            continue
        prompt = f"""EDGE AUDIT:
  Relation : {src} → {tgt}
...
Is this edge weight change merit-based or biased?"""
        ...
        try:
            ...
            if audit.get("is_biased"):
                ...
                edge_score["synergy_score"]  = new_w
            ...
            edge_score["p35_audited"]   = True
            edge_score["p35_is_biased"] = False
            edge_score["p35_reason"]    = "Audit failed"
    return audited


def run_phase3_5_bias_audit(phase3_output: dict, graph_data: dict,
                             job_desc: str, cv_text: str,
                             label: str = "", llm=None) -> dict:
    if llm is None:
        llm = make_llm_client()
    ...
    nodes_to_audit = [
        ...
        and abs(
            ...
            - float(ideal_nodes[n_id].get("w_0", 0))
...
def run_phase3_5(
    ...
    if not PHASE3_STORE:
        print("  ❌ PHASE3_STORE vide — lancez run_phase3 d'abord")
        return {}
    ...
    for pair_key, phase3_output in PHASE3_STORE.items():
        ...
        row_iloc = JOB_ROW_INDEX.get(job_id)
        if row_iloc is None:
            m = re.match(r"^job_(\d+)$", job_id)
            if m:
                row_iloc = int(m.group(1))
            else:
                print(f"  ⚠ {pair_key} — job_id '{job_id}' introuvable dans JOB_ROW_INDEX")
                continue

        if row_iloc >= len(df):
            print(f"  ⚠ {pair_key} — index {row_iloc} hors bornes ({len(df)} lignes)")
            continue
    ...
    print(f"\n  ✅ Phase 3.5 : {len(results)} paires auditées → PHASE3_STORE mis à jour")
    return results
...
def _w0(node: dict) -> float:
    return float(node.get("w_0", node.get("w_0", 0.5)))
def _weight(edge: dict) -> float:
    return float(edge.get("weight", edge.get("weight", 0.5)))
...
def _edge_wcv_norm(edge: dict, scored_edges: Dict[str, dict]) -> float:
    src = edge.get("source","")
    tgt = edge.get("target","")
    ed  = _lookup_edge(scored_edges, src, tgt)
...
def compute_weighted_gap(ideal_nodes: List[dict], cv_scores: Dict[str, dict]) -> dict:
    ...
    for node in ideal_nodes:
        ...
        gap  = max(0.0, w0 - wcvn)
        severity = ("critical" if gap >= 0.6 else "high" if gap >= 0.4
                    else "medium" if gap >= 0.2 else "low")
        match_pct = round(max(0.0, (1.0 - gap) * 100), 1)
        weighted_sum += gap * w0
        weight_total += w0
        entry = {
            "node_id": node_id,"label": node.get("label", node_id),
            "type": node.get("type",""),"w_0": round(w0, 3),
            "w_cv_normalized": round(wcvn, 3),"gap": round(gap, 3),
    ...
    return {
        "overall_match_pct": overall_match,"avg_weighted_gap": avg_weighted_gap,
...
def compute_cluster_scores(ideal_nodes: List[dict], cv_scores: Dict[str, dict]) -> Dict[str, dict]:
    clusters: Dict[str, list] = defaultdict(list)
    for node in ideal_nodes:
        ntype = node.get("type","hard_skill")
    ...
    for ntype, pairs in clusters.items():
        ...
        result[ntype] = {
            "count":      len(pairs),
            "match_pct":  round((wcv_sum / w0_sum) * 100, 1) if w0_sum else 0.0,
            "weight_sum": round(w0_sum, 3)}
    return result

def compute_edge_metrics(edges: List[dict], scored_edges: Dict[str, dict]) -> dict:
    edge_scores_list: List[dict] = []
    for edge in edges:
        source  = edge.get("source","")
        target  = edge.get("target","")
        w0      = _w0(edge)
        wcvn    = _edge_wcv_norm(edge, scored_edges)
        wcv_raw = wcvn * _MAX_EDGE_SCORE
        ed = _lookup_edge(scored_edges, source, target)
        edge_scores_list.append({
            "edge_key":      _normalize_edge_key(source, target),
            "source":        source,"target": target,
            "w_0":           round(w0, 3),"w_cv": round(wcv_raw, 3),
...
def compute_confidence(ideal_nodes: List[dict], cv_scores: Dict[str, dict],
                       gap_data: dict) -> dict:
    n = len(ideal_nodes)
    if n == 0:
        return {"confidence_score": 0.5,"confidence_label": "Low","factors": {}}
    nodes_audited = sum(1 for nd in ideal_nodes if
                        cv_scores.get(nd["id"],{}).get("p35_audited", nd.get("p35_audited", False)))
    nodes_biased  = sum(1 for nd in ideal_nodes if
                        cv_scores.get(nd["id"],{}).get("p35_is_biased", nd.get("p35_is_biased", False)))
    ...
    confident_from_cv = sum(
        1 for nid in [nd["id"] for nd in ideal_nodes]
    ...
    return {
        ...
        "confidence_label": ("High" if confidence >= 0.80 else
                             "Medium" if confidence >= 0.60 else "Low"),
        "factors": {
            "bias_rate": round(bias_rate, 3),"critical_rate": round(critical_rate, 3),
            "coverage_rate": round(coverage_factor, 3),
            "nodes_audited": nodes_audited,"nodes_biased": nodes_biased}}

def compute_profile_summary(ideal_nodes: List[dict], cv_scores: Dict[str, dict],
                             gap_data: dict) -> dict:
    strengths = []
    for node in ideal_nodes:
        nid  = node["id"]
        ...
        if wcvn >= 0.75 and w0 >= 0.6:
            strengths.append({
                ...
                "p3_confidence": cv_scores.get(nid,{}).get("p3_confidence","NONE")})
    weaknesses = [
        {"id": g["node_id"],"label": g["label"],"type": g["type"],
...
def extract_bias_summary(ideal_nodes: List[dict], edges: List[dict],
                         cv_scores: Dict[str, dict],
                         scored_edges: Optional[Dict[str, dict]] = None) -> dict:
    biased_nodes = []
    biased_edges = []
    ...
    for node in ideal_nodes:
        ...
        if is_biased:
            biased_nodes.append({
                "id": nid,"label": node.get("label", nid),
                "bias_category":     cv.get("p35_bias_category", node.get("p35_bias_category","")),
                "severity":          cv.get("p35_severity",       node.get("p35_severity","")),
                "correction_factor": cv.get("p35_correction_factor", node.get("p35_correction_factor",1.0)),
                "w_cv_before":       cv.get("w_cv", 0.0),
                "w_cv_corrected":    cv.get("w_cv_corrected",    cv.get("w_cv",0.0)),
                "reason":            cv.get("p35_reason",         node.get("p35_reason",""))})
    for edge in edges:
        source = edge.get("source","")
        target = edge.get("target","")
        sc = _lookup_edge(scored_edges, source, target)
        is_biased = sc.get("p35_is_biased", edge.get("p35_is_biased", False))
        if is_biased:
            biased_edges.append({
                "edge": _normalize_edge_key(source, target),
                "bias_category":     sc.get("p35_bias_category",""),
                "severity":          sc.get("p35_severity",""),
                "correction_factor": sc.get("p35_correction_factor",1.0),
                "w_cv_corrected":    sc.get("w_cv_corrected", sc.get("w_cv",0.0)),
                "reason":            sc.get("p35_reason","")})
    cats = Counter(
        n["bias_category"] for n in biased_nodes + biased_edges
        if n.get("bias_category") not in ("NONE",""))
    nodes_audited = sum(
        1 for nd in ideal_nodes
        if cv_scores.get(nd["id"],{}).get("p35_audited", False) or nd.get("p35_audited", False))
    edges_audited = sum(
        1 for e in edges
        if _lookup_edge(scored_edges, e.get("source",""), e.get("target","")).get("p35_audited", False)
        or e.get("p35_audited", False))
    return {
        "nodes_audited": nodes_audited,"nodes_with_bias": len(biased_nodes),
        "edges_audited": edges_audited,"edges_with_bias": len(biased_edges),
        "bias_categories": dict(cats.most_common()),
        "biased_nodes": biased_nodes,"biased_edges": biased_edges}

def compute_final_candidate_score(ideal_nodes: List[dict], edges: List[dict],
                                   cv_scores: Dict[str, dict],
                                   scored_edges: Dict[str, dict]) -> dict:
    node_scores, node_weights = [], []
    for node in ideal_nodes:
        w0 = _w0(node)
        node_scores.append(_wcv_norm(node["id"], cv_scores) * w0)
        node_weights.append(w0)
    edge_scores_w, edge_weights = [], []
    for edge in edges:
        w0  = _w0(edge)
        wcv = _edge_wcv_norm(edge, scored_edges)
        edge_scores_w.append(wcv * w0)
...
def compute_advanced_metrics_v6(ideal_nodes: List[dict], edges: List[dict],
                                 cv_scores: Dict[str, dict],
                                 scored_edges: Dict[str, dict]) -> dict:
    n = len(ideal_nodes)
    if n == 0:
    ...
    missing_critical_penalty = (
        float(sum(1 for nid in critical if nid in covered) / len(critical))
    ...
    return {
        ...
        "ndcg_5": ndcg_5,"ndcg_10": ndcg_10,"map_score": map_score,"mrr_score": mrr_score,
...
def _empty_v6_metrics() -> dict:
    return {k: 0.0 for k in [
        "node_matching_cosine","edge_matching_score","gap_score","jaccard_nodes",
        "weighted_precision","weighted_recall","weighted_f1","skill_depth_score",
        "missing_critical_penalty","redundancy_penalty","redundancy_info",
...
def compute_combo_matching(v6: dict) -> float:
    return round((0.40*v6.get("node_matching_cosine",0)+0.35*v6.get("edge_matching_score",0)+0.25*v6.get("jaccard_nodes",0))*100,2)
def compute_combo_fit(v6: dict) -> float:
    return round((0.40*v6.get("gap_score",0)+0.35*v6.get("semantic_fit_score",0)+0.25*v6.get("jaccard_nodes",0))*100,2)
def compute_combo_ranking(v6: dict) -> float:
    return round((0.40*v6.get("ndcg_5",0)+0.35*v6.get("map_score",0)+0.25*v6.get("mrr_score",0))*100,2)
def compute_combo_prf1(v6: dict) -> float:
    return round((0.40*v6.get("weighted_f1",0)+0.30*v6.get("skill_depth_score",0)+0.20*v6.get("weighted_recall",0)+0.10*v6.get("weighted_precision",0))*100,2)
def compute_combo_safe(v6: dict) -> float:
    return round((0.50*v6.get("missing_critical_penalty",0)+0.30*v6.get("gap_score",0)+0.20*v6.get("skill_balance_score",0))*100,2)
def compute_combo_specialist(v6: dict) -> float:
    return round((0.50*v6.get("top_skill_alignment",0)+0.30*v6.get("skill_depth_score",0)+0.20*v6.get("experience_density",0))*100,2)
...
def compute_pool_ranking(reports: Dict[str, dict]) -> Dict[str, dict]:
    ...
    metric_values = {m: [] for m, _ in RANKING_METRICS}
    for key in keys:
        for mn, ext in RANKING_METRICS:
            metric_values[mn].append(ext(reports[key]))
...
def phase4_generate_report(job_id: str, cv_id: str,
                            graph_data: dict, phase3_data: dict) -> dict:
    ...
    if fallback_ratio > 0.3:
        print(f"    ⚠ fallback_ratio : {fallback_ratio:.1%}")
    ...
    return {
        ...
        "advanced_metrics": {
            "NTCW": {"value": ntcw,
                     ...
                     "interpretation": ("Sur-qualifié" if ntcw > 1.05 else
                                        "Aligné" if ntcw >= 0.80 else "Sous-qualifié")},
            "CTM":  {"value": ctm,
                     "description": "Contextual Transformation Magnitude — distance structurelle",
                     "interpretation": ("Profil proche" if ctm <= 0.25 else
                                        "Écart modéré" if ctm <= 0.50 else "Profil très différent")},
        ...
        "advanced_metrics_v6": {
            "node_matching_cosine":     {"value": v6_metrics["node_matching_cosine"],
                                         "description": "Matching nœuds pondéré par importance JD",
                                         "interpretation": ("Fort" if v6_metrics["node_matching_cosine"] >= 0.7
                                                            else "Moyen" if v6_metrics["node_matching_cosine"] >= 0.4
                                                            else "Faible")},
            "edge_matching_score":      {"value": v6_metrics["edge_matching_score"],
                                         "description": "Matching des relations/synergies JD→CV",
                                         "interpretation": ("Bon" if v6_metrics["edge_matching_score"] >= 0.6
                                                            else "Partiel")},
            "gap_score":                {"value": v6_metrics["gap_score"],
                                         "description": "1 - distance L2 normalisée vs profil idéal (1=parfait)",
                                         "interpretation": ("Proche" if v6_metrics["gap_score"] >= 0.7
                                                            else "Éloigné")},
            "jaccard_nodes":            {"value": v6_metrics["jaccard_nodes"],
            ...
            "weighted_f1":              {"value": v6_metrics["weighted_f1"],
                                         "description": "F1 harmonique pondéré par importance JD",
                                         "interpretation": ("Fort" if v6_metrics["weighted_f1"] >= 0.6
                                                            else "Faible")},
            "skill_depth_score":        v6_metrics["skill_depth_score"],
            "missing_critical_penalty": {"value": v6_metrics["missing_critical_penalty"],
                                         "description": "Proportion de skills critiques (w_0≥0.7) couverts",
                                         "interpretation": ("Skills critiques couverts"
                                                            if v6_metrics["missing_critical_penalty"] >= 0.7
                                                            else "Skills critiques manquants — risque élevé")},
            "redundancy_penalty":       {"value": v6_metrics["redundancy_penalty"],
                                         "description": "Pénalité surqualification inutile (1=aucune redondance)"},
            "redundancy_info":          {"value": v6_metrics["redundancy_info"],
                                         "label": ("Profil bien calibré" if v6_metrics["redundancy_info"] < 0.1
                                                   else "Surqualification partielle"),
        ...
        "combo_scores_v6": {
            "combo_v6_final":   {"value": combo_v6_final,
            ...
            "combo_specialist": {"value": combo_specialist,
        ...
        "profile_summary": {
            ...
            "critical_weaknesses": [{"label":w["label"],"type":w["type"],"gap":w["gap"]}
        ...
        "decision_rationale": {
            "why_this_verdict": (
                f"NTCW={ntcw} ({'sous-qualifié' if ntcw < 0.8 else 'aligné'}), "
            ...
            "scoring_reliability": (
                f"⚠ {fallback_ratio:.1%} des scores via fallback"
...
def generate_executive_summary(reports: Dict[str, dict]) -> str:
    ...
    for r in sorted(reports.values(), key=lambda r: r.get("pool_ranking",{}).get("rank",9999)):
        ...
        cv6     = _safe_val(r.get("combo_scores_v6",{}).get("combo_v6_final",0))
        wf1     = _safe_val(r.get("advanced_metrics_v6",{}).get("weighted_f1",0))
        gap     = _safe_val(r.get("advanced_metrics_v6",{}).get("gap_score",0))
        conf    = r["overall_assessment"]["confidence"]["confidence_label"]
        lines.append(f"| #{rank} | {pair} | {tier} | {verdict} | {match}% | {cv6} | {wf1} | {gap} | {conf} |")
    return "\n".join(lines)


def run_phase4(test_mode: bool = TEST_MODE, output_dir: str = REPORTS_DIR) -> Dict[str, dict]:
    print("\n" + "█" * 70)
    print("█  PHASE 4 — SCORING FINAL & RAPPORT")
    print("█" * 70)

    if not PHASE3_STORE:
        print("  ❌ PHASE3_STORE vide — lancez run_phase3 (et run_phase3_5) d'abord")
        return {}
    ...
    for pair_key, phase3_data in PHASE3_STORE.items():
        ...
        if not graph_data:
            print(f"  ⏩ {pair_key} — graphe manquant")
            continue
...
def run_full_pipeline(
    ...
    t_start = time.time()
    print("\n" + "═" * 70)
    print(f"  PIPELINE — {'🧪 TEST' if test_mode else '🚀 PROD'}")
    print(f"  CSV : {csv_path}")
    print(f"  Plage : lignes {start_row} → {end_row-1}  ({end_row - start_row} jobs)")
    print(f"  job_ids : job_{start_row} … job_{end_row-1}")
    print(f"  CVs/job : {max_cvs}")
    print("═" * 70)

    if run_graph:
        run_phase1_graph_generation(
            csv_path=csv_path, test_mode=test_mode,
            start_row=start_row, end_row=end_row)

    if run_p2:
        run_phase2(
            csv_path=csv_path, test_mode=test_mode,
            max_cvs=max_cvs, start_row=start_row, end_row=end_row)
